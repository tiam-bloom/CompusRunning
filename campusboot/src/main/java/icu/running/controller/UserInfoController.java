...
public class UserInfoController {
    ...
    @ResponseBody
    public String addPicture(UserInfo userInfo,HttpServletRequest req,Model model){
        User user = (User) req.getSession().getAttribute("USER_SESSION");
        userInfo.setId(user.getId());
        String tip;
        int a = userInfoService.upBigHead(userInfo);
        if (a>0){
            //model.addAttribute("Tohome","上传成功!单击跳转");
            tip = "上传成功!单击跳转";
        }else {
            //model.addAttribute("Tohome","上传失败!");
            tip = "上传失败!";
        }
        return tip;
    }
    ...
    public String home(HttpServletRequest req, Model model) {
        User user = (User) req.getSession().getAttribute("USER_SESSION");
//        System.out.println("大傻子哈哈"+user.getId());
